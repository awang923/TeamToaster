    ...
    def __init__(self, parent, controller):
        ...
        def compute():
            ...
            if globals.op == 'transfer':
                root = Node(globals.ship, buffer_init,
                            globals.unload_list, 'transfer')
                initial_time = time.time()
                goal = search(root)
                unload_buffer_node = unload_buffer(goal)
                load_ship_node = load_ship(
                    unload_buffer_node, globals.load_list)
                # print(load_ship_node.state)
                compute_time = (time.time() - initial_time) * 1000
                compute_time_label = Label(
                    self, text="Total Time Computing = " + str(compute_time) + "ms")
                ...
                for i in order_of_operations(load_ship_node):
                    ...
                    # print(i)
                moves_time = Label(
                    self, text="Estimated Time to Perform Moves = " + str(load_ship_node.g) + "min")
                ...
                for i in order_of_operations(unload_buffer_node):
                    ...
                    # print(i)
                moves_time = Label(
                    self, text="Estimated Time to Perform Moves = " + str(unload_buffer_node.g) + "min")
                moves_time.place(relx=.5, rely=step_y, anchor=CENTER)
                globals.operations_list = order_of_operations(
                    unload_buffer_node)
        ...
        compute_button = Button(
            self, text="COMPUTE STEPS", command=lambda: compute())
        ...
        done_button = Button(
            self, text="DONE", command=lambda: controller.show_frame(Operation))
