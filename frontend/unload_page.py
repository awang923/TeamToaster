...
class InputUnloadPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        unload_prompt = Label(
            self, text="Please input all containers to be unloaded")
        ...
        def on_click(container_name, mass, x_coord, y_coord):
            print("[" + x_coord + ", " + y_coord + "] " +
                  container_name + ", " + mass)
            confirm_unload.config(text=container_name +
                                  ", " + mass + "kg has been entered")
                    ...
                    for line in manifest_lines:
                        ...
                        if (regex_matches.group(1).lstrip('0') == str(temp) and regex_matches.group(2).lstrip('0') == str(y+1)):
                            if regex_matches.group(4) == "NAN":
                                b = Button(table_frame, text=regex_matches.group(
                                    4), height=3, width=6, highlightbackground='#CEBBBB', state="disabled")
                                tooltipString = b.cget('text')
                            elif regex_matches.group(4) == "UNUSED":
                                b = Button(table_frame, text=regex_matches.group(
                                    4), height=3, width=6, highlightbackground='white', state="disabled")
                                tooltipString = b.cget('text')
                            else:
        ...
        view_grid = Button(self, text="VIEW GRID",
                           command=lambda: view_grid_click())
        ...
        done_button = Button(
            self, text="DONE", command=lambda: done_button_click())
