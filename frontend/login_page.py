...
class LoginPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        intro_prompt = Label(
            self, text="Welcome to Team Toaster's Application")
        ...
        sign_in_button = Button(self, text="Sign In",
                                command=lambda: sign_in_on_click(), width=20)
        sign_in_button.place(relx=.5, rely=.25, anchor=CENTER)

        test_button = Button(
            self, text="test", command=lambda: controller.show_frame(Operation))
